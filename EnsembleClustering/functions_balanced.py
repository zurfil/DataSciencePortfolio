...
def _select_best_members(data, labels, target_n_members=25):
    """
    Selects the best algorithms from ensemble member pool based on their average ranking across two metrics:
    Silhouette Score and NMI Score, then it moves them to target pool.
    ...
    """
    num_algorithms = labels.shape[1]
    member_pool = list(range(num_algorithms))
    target_pool = []

    # Move the algorithm with the highest Silhouette score from member_pool to target_pool
    silhouette_scores = [silhouette_score(data, labels[:, i]) for i in member_pool]
    best_index = np.argmax(silhouette_scores)
    target_pool.append(best_index)
    member_pool.remove(best_index)

    while len(target_pool) < target_n_members:
        silhouette_rankings = []
        nmi_rankings = []

        # Compute silhouette scores and rankings for algorithms in member_pool
        silhouette_scores = [silhouette_score(data, labels[:, i]) for i in member_pool]
        silhouette_order = np.argsort(silhouette_scores)[::-1]
        for i in range(len(member_pool)):
            silhouette_rankings.append(np.where(silhouette_order == i)[0][0] + 1)

        # Compute NMI rankings for algorithms in member_pool
        for i in member_pool:
            nmis = [normalized_mutual_info_score(labels[:, i], labels[:, j]) for j in target_pool]
            mean_nmi = np.mean(nmis)
            nmi_rankings.append(mean_nmi)
        nmi_order = np.argsort(nmi_rankings)
        for i in range(len(member_pool)):
            nmi_rankings[i] = np.where(nmi_order == i)[0][0] + 1

        # Compute average rankings
        avg_rankings = [(silhouette_rankings[i] + nmi_rankings[i]) / 2 for i in range(len(member_pool))]

        # Select the algorithm with the highest average rank
        best_avg_index = np.argmin(avg_rankings)
        target_pool.append(member_pool.pop(best_avg_index))

    return labels[:, target_pool]
...
def make_test_balanced(datasets, noise_levels):
    """
    ...
    then extracts the results into a .csv file named "results_balanced_total.csv".
    ...
    total_results_df.to_csv(f"results/results_balanced_total.csv", sep=',', index=False)
